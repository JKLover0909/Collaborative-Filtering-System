...
for i, row in pt.iterrows():
    row_vector = row.tolist()
    row_vectors.append(row_vector)
num_row = len(row_vectors)
num_col = len(row_vectors[1])
sqr_rows = []
for i in row_vectors :
    sqr = 0
    for j in i:
        sqr += j*j
    sqr_rows.append(math.sqrt(sqr))

pt_cosin = []
for i in row_vectors :
    pt_cosin_row = []
    cosin = 0
    for j in row_vectors :
        if i == j :
            cosin = 1
        else :
            cosin = sum(i[k]*j[k] for k in range(num_col)) / (sqr_rows[row_vectors.index(i)] * sqr_rows[row_vectors.index(j)]) 
        pt_cosin_row.append(cosin)
    pt_cosin.append(pt_cosin_row)
pt_cosin
pt_cosin_df = pd.DataFrame(pt_cosin, columns=popular_books, index=popular_books)
pt_cosin_df

def recommend(book_name):
    index = np.where(pt_cosin_df.index==book_name)[0][0]
    similar_books = sorted(list(enumerate(pt_cosin[index])),key=lambda x:x[1], reverse=True)[1:6]
    
    data = []
    
    for i in similar_books:
        item = []
        temp_df = books[books['Book-Title'] == pt_cosin_df.index[i[0]]]
